...
export const getDataFromServer = ({ dispatch, getState }) =>
  (next) =>
    (action) => {
    ...
    (next) =>
      (action) => {
        if (action.type === "GET_PLANETS") {
          ...
          const existsPlanet = getState().planetsReducer.planets[parseInt(action.payload)];
          return existsPlanet
            ? new Promise((resolve) => resolve(existsPlanet))
            : dispatch(actions.getDataFromServer(`planets/${action.payload || ""}`))
              .then((data) => {
                let resObj = {
                  name: data.name,
    ...
    (next) =>
      (action) => {
        if (action.type === "GET_PEOPLES") {
          // get the planet and save in redux, to avoid unnecessary api calls
          // if it exist in redux- dont use fetch
          const existsPeople = getState().peoplesReducer.peoples[parseInt(action.payload)];
          return existsPeople
            ? new Promise((resolve) => resolve(existsPeople))
            : dispatch(actions.getDataFromServer(`people/${action.payload || ""}`))
              .then((data) => {
                let resObj = {
                  name: data.name,
