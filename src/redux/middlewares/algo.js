        ...
        (next) =>
            (action) => {
                if (action.type === "GET_MAX_VEHICLE") {
                    return dispatch(actions.getDataFromServer(`vehicles/${action.payload || ""}`))
                        .then((data) => {
                            data.results.forEach((vehicle) => { //check every vehicle data
                                ...
                                vehicle.pilots.forEach((pilot) => { //looking for every pilot's planet
                                    ...
                                    dispatch(actions.getPeoples(pilotId)).then((people) => {
                                        if (people)
                                            dispatch(actions.getPlanets(people.homeworld)).then(
                                                (planet) => {
                                                    if (!calcPlanets[parseInt(people.homeworld)]) { //if this planet didnt calculate yet 
                            ...
                            if (data.next) { // looking for the next page of vehicles
                                dispatch(actions.getMaxVehicle(data.next.split("/")[5]));
                            }
                            else 
                                dispatch(actions.setMaxFinished(true));
                        });
                }
                return next(action);
            };
        ...
        (next) =>
            (action) => {
                if (action.type === "GET_PLANETS_TO_CHART") {
                    ...
                    if (planetsToFind.length > 0)// if there is something to find
                        dispatch(actions.getDataFromServer(`planets/${next || ""}`)) //get list data from server
                            .then((data) => {//find every planet name in every planet in results
                                data.results.forEach((planet) => {
                                    let findPlanet = planetsToFind.find(p => p === planet.name);
                                    if (findPlanet) {
                                        dispatch(actions.addPlanet({
                                            key: parseInt(planet.url.split("/")[5]),
                                            value: { name: planet.name, residents: planet.residents?.length },
                                        }));
                                ...
                                if (data.next)//looking for next page of results
